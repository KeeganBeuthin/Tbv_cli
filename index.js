...
program
  ...
  .option(
    "-l, --language <type>",
    "programming language of the source file (java, python, typescript)"
  );
...
const preprocessCFile = (filePath) => {
  ...
  content = content.replace(
    /^#include <corecrt_wstdio.h>$/m,
    "#include <stdarg.h>\n#include <corecrt_wstdio.h>"
  );
  content = content.replace(
    /^#define __crt_va_start(.*)$/m,
    "#ifndef __crt_va_start\n#define __crt_va_start va_start\n#endif"
  );
  content = content.replace(
    /^#define __crt_va_end(.*)$/m,
...
const getNuitkaIncludePath = () => {
  const includePath = execSync(
    'python -c "import nuitka; import os; print(os.path.dirname(nuitka.__file__))"'
  )
    .toString()
    .trim();
...
const convertPythonToWasm = (file) => {
  ...
  const intermediateBuildDir = path.join(
    buildDir,
    `${fileNameWithoutExtension}.build`
  );
  const outputWasmFile = path.join(
    buildDir,
    `${fileNameWithoutExtension}.wasm`
  );
  ...
  try {
    ...
    const windowsKitIncludePath =
      "C:/Program Files (x86)/Windows Kits/10/Include/10.0.22621.0/ucrt";
    const visualStudioIncludePath =
    ...
    const emccArgs = [
      ...
      "-s",
      'EXPORTED_FUNCTIONS=["_execute_credit_leg","_execute_debit_leg","_http_request"]',
      "-s",
      'EXPORTED_RUNTIME_METHODS=["ccall","cwrap"]',
      "-fdeclspec",
      "-fms-extensions",
      "-D_CRT_STDIO_INLINE=__inline",
      '-D__int64="long long"',
      "-D__crt_va_start=va_start",
      "-D__crt_va_end=va_end",
    ];
    ...
    console.log(
      `Successfully converted ${file} to WebAssembly at ${outputWasmFile}`
    );
  } catch (err) {
...
const convertToWasm = (file, language) => {
  ...
  switch (language.toLowerCase()) {
    case "java":
      ...
      executeCommand("java", [
        "-jar",
        "jwebassembly-cli.jar",
        file,
        "-o",
        `${path.basename(file, path.extname(file))}.wasm`,
      ]);
      break;
    case "python":
    ...
    case "typescript":
      executeCommand("asc", [
        ...
        `${path.basename(file, path.extname(file))}.wasm`,
        "-t",
        `${path.basename(file, path.extname(file))}.wat`,
      ]);
      break;
    default:
      console.error(
        `Unsupported language for WebAssembly conversion: ${language}`
      );
  }
};
...
const executeJavaTests = (file) => {
  ...
  try {
    ...
    execSync(`mvn exec:java -Dexec.mainClass="${className}"`, {
      stdio: "inherit",
    });
  } catch (err) {
...
program
  ...
  .action(() => {
    ...
    if (!language) {
      console.error(
        "Please provide a programming language using the -l option."
      );
      process.exit(1);
    }
...
program
  ...
  .action(() => {
    ...
    switch (language.toLowerCase()) {
      ...
      default:
        console.error(
          `Unsupported language. Please choose java, python, typescript, or wasm.`
        );
    }
  });
